    ...
    std::optional<std::vector<int>> filament_maps_new)
{
    ...
    if (num_filaments <= 1) {
        ...
        if (!darkmoon_applied) {
            BOOST_LOG_TRIVIAL(info) << "PresetBundle: Darkmoon dynamic config skipped; running fallback apply_dynamic_darkmoon_bed_temps_if_not_user_modified (single filament case)";
            apply_dynamic_darkmoon_bed_temps_if_not_user_modified(filament_config, extruder_count);
        }
        ...
        for (size_t i = 0; i < num_filaments; ++i) {
            ...
            if (!darkmoon_applied) {
                BOOST_LOG_TRIVIAL(info) << "PresetBundle: Darkmoon dynamic config skipped; running fallback apply_dynamic_darkmoon_bed_temps_if_not_user_modified (multi-filament case, filament " << i << ")";
                apply_dynamic_darkmoon_bed_temps_if_not_user_modified(filament_temp_configs[i], extruder_count);
            }
            if (apply_extruder)
    ...
    if (!darkmoon_applied_out) {
        BOOST_LOG_TRIVIAL(info) << "PresetBundle: Darkmoon dynamic config skipped; running fallback apply_dynamic_darkmoon_bed_temps_if_not_user_modified (aggregated filament config)";
        apply_dynamic_darkmoon_bed_temps_if_not_user_modified(out, extruder_count);
    }
...
DynamicPrintConfig PresetBundle::full_fff_config(bool apply_extruder, std::optional<std::vector<int>> filament_maps_new) const
{
    ...
    if (!darkmoon_applied_out) {
        BOOST_LOG_TRIVIAL(info) << "PresetBundle: Darkmoon dynamic config skipped; running fallback apply_dynamic_darkmoon_bed_temps_if_not_user_modified (assembled config)";
        apply_dynamic_darkmoon_bed_temps_if_not_user_modified(out, extruder_count);
    }
